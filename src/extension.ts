...
class PythonWorker {
    ...
    async ensureStarted(): Promise<void> {
        ...
        if (!fs.existsSync(pyExe)) {
            await vscode.window.withProgress(
                { location: vscode.ProgressLocation.Notification, title: 'Mobius', cancellable: false },
                async (progress) => {
        ...
        this.process = spawn(pyExe, ['worker.py'], {
            cwd: backendPath,
    ...
    private runProcess(cmd: string, args: string[], cwd: string): Promise<number> {
        return new Promise((resolve) => {
            const proc = spawn(cmd, args, { cwd, shell: true });
            proc.stdout?.on('data', (d: Buffer) => this.outputChannel.appendLine(d.toString()));
            proc.stderr?.on('data', (d: Buffer) => this.outputChannel.appendLine(d.toString()));
...
class MobiusSidebarProvider implements vscode.WebviewViewProvider {
    ...
    private getSidebarHtml(webview: vscode.Webview, nonce: string): string {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('mobius.sidebarView', sidebarProvider)
    );

    // Register dashboard command
    context.subscriptions.push(
        vscode.commands.registerCommand('mobius.openDashboard', async () => {
            try {
